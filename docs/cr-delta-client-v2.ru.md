...
- **OQ-1 (дубликаты в бесключевых): РЕШЕНО — нет.** Клиент гарантирует отправку только уникальных дельт. Для бесключевых таблиц он делает сравнение множеств и эмитит **только INSERT / DELETE** (несовпавшая строка → DELETE, новая → INSERT); **UPDATE для бесключевых не производится** (подтверждает §6). Full-row-ключ считается уникальным → **счётчик кратности не нужен**.
