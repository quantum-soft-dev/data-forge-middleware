...
- **OQ-1 (дубликаты в бесключевых):** содержат ли бесключевые таблицы байт-идентичные дубликаты строк? Если да, нужен счётчик кратности строки (§6); если нет, full-row-ключ считается уникальным. *Ждёт ответа.*
...
- **OQ-3 (тип сайта / флаг приёма):** как помечать сайт как «Delta v2 ingestion» — переиспользовать `site_type`, добавить флаг `ingestion_protocol` или выводить по первой gRPC-сессии. *Решить до Фазы 1.*
