...
- **Full per-table Parquet load** (reinstated in feature 023, B3): each checkpoint build also materializes `checkpoints/{siteId}/{table}/seq={seq}/snapshot.parquet` — the full typed snapshot per table (`s3_key_parquet`, V36) — for tables with a declared schema. Parquet is the target serving format (segment deltas + full loads); the checkpoint CSV remains the legacy Bit BI path. Both are downloadable from the Delta Sync UI via presigned URLs.
- _Retired_: the `_change_date` change-feed partitions (original T4.3 design) — superseded by the delta stream. (The checkpoint floor `snapshot.parquet` was retired in T8.5 and reinstated in 023/B3 as the full-load artifact.)
