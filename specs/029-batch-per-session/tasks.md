...
- [ ] **T001** Migration V40 + Batch activity field.
...
- [ ] **T003** Split segment commit from batch completion in `DeltaSessionCommitService`.
...
- [ ] **T004** Ingestion lifecycle: seal keeps the batch; no tail batch; no per-seal batch cycling.
...
- [ ] **T005** List-view aggregation across a batch's segments.
...
- [ ] **T006** Activity touch on the ingest path.
...
- [ ] **T007** Sweeper cutoff on `COALESCE(last_activity_at, started_at)`.
...
- [ ] **T008** End-to-end integration suite (Testcontainers + LocalStack), `src/test/java/integration/BatchPerSessionIngestionIT.java` (align naming with existing integration suites):
...
- [ ] **T009** Docs: `docs/cr-batch-per-session.md` (motivation incl. the fyt-new incident, semantics, D1–D7, migration V40, ops notes: rollback behavior, timeout semantics); update `docs/delta-client-v2-guide.md` (batch = session; continuous slicing invisible in history) and `CLAUDE.md` (Recent Changes entry, migration pointer V40 → next V41).
