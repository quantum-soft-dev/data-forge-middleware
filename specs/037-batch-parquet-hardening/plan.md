...
1. Completion enqueue runs in a new transaction after the ingestion transaction commits, then
   wakes the worker. Callback failures are contained and lazy backfill remains the recovery path.
   When any legacy segment
...
2. Admin batch deletion delegates to a transactional application service. Unified artifact key
   ...
   is one transaction; S3 cleanup remains best effort and starts only after that transaction commits.
