...
| A3 | Tailwind theme `tailwind.config.js:22-77` | Only Geist font + radius mapping; no monitoring colors/shadows — components hardcode hexes | Semantic utilities (research D1): `shadow-panel/card-inner/icon-circle`, `ink.*`, `brand.*`, `border-hairline/separator` | Extend `theme.extend`; no removals (old utilities die with their usages in Phase C) | S | after A1 |
