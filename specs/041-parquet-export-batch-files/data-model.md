...
| index `idx_batch_parquet_artifacts_catalog_ready` | `(ready_at, s3_key) WHERE READY` | | READY branch |
| index `idx_batch_parquet_artifacts_catalog_abandoned` | `(updated_at, 'abandoned/'||id) WHERE ABANDONED` | | ABANDONED branch |
