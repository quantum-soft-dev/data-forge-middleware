...
- **Board and GraphQL budget**: 5000 GraphQL points an hour per account, shared by every session. Never `gh project …`; move cards and read the board only through `scripts/board.sh` (`status`/`field` 3 points, `list`/`items`/`sweep` 1 per 100 cards) and read labels, bodies, comments and state through REST (`gh api repos/…`, 0 points). `status: *` labels move only through `board.sh` — a direct label edit leaves the column behind — and a refusal of `board.sh` is a stop, not a manual workaround. Assign before moving a card, re-read the column it prints, retry once. Poll CI with `--interval 60`; read the budget with `scripts/wave.sh budget`, never `gh api rate_limit` (it reports `used: 0` for GraphQL). `scripts/board.sh sweep` finds board drift; `--fix` repairs stale blocks, columns (by label) and closed tickets. Full table in **`CLAUDE.md`** → "GraphQL budget".
- **Parallel runs**: `/wave [N]` runs up to **three** tickets at once with `task-runner` subagents in `.claude/worktrees/<n>-<slug>`, refills freed slots and merges one at a time; `scripts/wave.sh` computes the wave — pool `Backlog`+`Ready`, order by `priority: *`, no two tickets sharing a key from «Что тронет» (file names, `flyway`, `specs`, `proto`, `stand`) — and keeps a journal it verifies against the board. `/github-issue-runner` keeps the Conductor-workspace mode. Details: **`CLAUDE.md`** → "Running several issues at once", `docs/cr-wave-dispatcher.md`.
...
| `SQL_GENERATION_ADOPTED` | This attempt lost the unique claim and adopted the winner's generation (issue #260) |
...
- wave-dispatcher: `/wave` — an autonomous dispatcher whose planning is a script, ported from zmanly
  ...
  (plan/refill/begin/add/mark/status/verify/end/budget/health) and `scripts/board.sh` `items`,
  `field <n> Size`, `sweep [--fix]`. The ceiling stays **three**, the pool is `Backlog` + `Ready` with
  any milestone, priority is the `priority: *` label, overlap keys come from «Что тронет» (file names
  plus `flyway`/`specs`/`proto`/`stand`), each ticket's base is `issue-base.sh`'s answer, merge goes
  through `pr-merge.sh`, and a rebase renumbers a Flyway migration a neighbour took. The budget rule
  stops recommending `gh api rate_limit` (it reports `used: 0` for GraphQL after points are charged) in
  favour of `scripts/wave.sh budget`; `/task` reads a block in three cases, assigns before moving a
  card, re-reads the printed column and returns the card to `Blocked`/`Ready` before reporting a stop.
  Tests: `BoardScriptTest` (items, field, sweep) and new `WaveScriptTest`, both mutation-proven. No
  production code, migration (**V60 stays next**) or contract change. The full entry is in `CLAUDE.md`;
  see `docs/cr-wave-dispatcher.md`.
...
- migration-base-branch: A ticket can land somewhere other than `develop` (issue #298). The Spring
  ...
  configuration-key, metric, S3-key or frontend change.
- review-command-issue-templates: `/review [pr#] [issue#] [--comment]` runs `/task`'s three review lenses on their own and reports one table (verdict, SERIOUS in full, MINOR by count) — no fixes, no merge, no board move. It fills the "`review` skill" that `CLAUDE.md`, `/github-issue` step 8 and `/merge` named for later review rounds and that did not exist. A lens without a `VERDICT:` line is *no verdict*, never READY; READY covers the lenses only, not CI, review threads or Conductor diff comments; `--comment` posts the table as one PR comment. Issue forms in `.github/ISSUE_TEMPLATE/` (`task`, `bug`, `decision`; blank issues off) carry the follow-up rule's three lines as fields and add the card to project 16; `gh issue create --body` gets no form, so an agent writes the same sections by hand. `scripts/board.sh` never set a `status:*` label from #277 on — `strip_status_labels` read into a non-`local` `label` and emptied `set_status`'s — so `board.sh unblock`, which selects by `status: blocked`, never unblocked anything; one `local` fixes it (found filing the Spring Boot 4.1 migration tickets #298–#305). No production code, REST, gRPC, proto, DTO, migration, configuration-key, metric, S3-key or frontend change.
...
- commit-gate-outside-ci: The per-task gate stopped lying in both directions (issue #278, folding
  **#279**; both found finishing #205/PR #255, both claimed `build.gradle.kts`, so they could never
  ...
  `ChangelogSegmentQueueMarkerClobberTest` (#245) failed two methods, deterministically outside UTC,
  on a branch whose diff was five markdown files — and the gate is mandatory with `--no-verify`
  forbidden, so a docs-only commit cost a cycle chasing somebody else's test (#207/#226's class).
  **The ticket's diagnosis was wrong**: it named the read (`rs.getTimestamp`) and its prescribed
  `getObject(col, LocalDateTime.class)` is a **no-op** — `getTimestamp(...).toLocalDateTime()` is a
  net identity. The shift is the **write**: `hibernate.jdbc.time_zone: UTC` makes Hibernate read a
  bound `LocalDateTime` as JVM-local wall clock and store the instant in UTC. Production is symmetric
  (writes and reads through Hibernate; the columns are compared against the database's own
  `CURRENT_TIMESTAMP`); this class deliberately reads the **row** through `JdbcTemplate` (#245's
  reason), where it is not. Now binds a JVM wall clock as production does and expects the stored form
  (`asStored`, identity in UTC); green under `TZ=UTC`, `Asia/Jerusalem`, `America/Los_Angeles`,
  `Australia/Sydney`. Pinning the test JVM's zone (option 3) **not taken** — a build-wide invariant
  that hides this class rather than showing it. No other `getTimestamp` + `LocalDateTime` in
  `src/test`; the three in `ParquetExportCatalogDao` are production and are **#280**. **Part A —
  green where something is broken**: the journal ships to both files, and nothing held it — the
  omission is invisible in a PR touching one file. #205 fixed one miss and answered "review habit";
  four days later three more were missing (`double-nan-sql-literal`, `adopt-path-side-effects`,
  `signed-nan-classification`). The journals are unequal, but 13 of the 16 differences are the block
  `AGENTS.md` stopped carrying and resumed, and entries are only **prepended**, so the anchor
  `042-parquet-phase-metrics` never moves — one constant, the `MigrationDocumentationConsistencyTest`
  (#104) shape. `AgentJournalConsistencyTest` asserts presence above the anchor, no duplicate within
  either file (#205 found `split-scratch-ceilings` twice, the copies differing in the clause #153 made
  stale), and — added here — the same relative **order** in both, an entry backfilled into the wrong
  place being the same defect one step quieter; that found a pre-existing inversion
  (`split-scratch-ceilings` ahead of `ingestion-commit-no-s3` while #147 is newer than #138), fixed by
  moving the entries. All three mutation-proven, red before the backfill on exactly three slugs.
  `build.gradle.kts` already declared both documents as test inputs; the hook's docs-only branch runs
...
- unpaired-sql-started: A lost SQL-generation unique claim writes `SQL_GENERATION_ADOPTED` so
  ...
  the gap #246 named and left). New action type, V57 widens `chk_plugin_audit_logs_action_type`;
  metadata names the adopted generation and the winner's key; Logs tab label "SQL Already
...
- failing-first-checkpoint: A first checkpoint build that keeps failing is no longer the same
  ...
  would have reported the largest first `FULL_SNAPSHOT` as critical on day one. **Shape 2**: V56
...
- double-nan-sql-literal: A non-finite `double` reaches Bit BI as a quoted literal, so the SQL is
  valid PostgreSQL (issue #233). `SqlStatementGenerator.formatJsonValue` rendered every `Number`
  through `toString()`, so a `real`/`double precision` cell holding `NaN`/`±Infinity` — which the
  extractor sends as `double_value` — produced `SET price = NaN`, where a bare `NaN` is an
  **identifier**: `ERROR: column "nan" does not exist`. **Invisible on this side**, which separates
  it from #215: generation succeeded, the file reached S3, the batch was marked processed, and the
  error surfaced only when Bit BI applied the file — taking the rest of it with it. Emitted as
  `'NaN'`/`'Infinity'`/`'-Infinity'`, which PostgreSQL coerces to the column's type. **Quoted rather
  than NULLed, and the asymmetry with #215 is the decision**: Parquet DECIMAL cannot hold the value
  (so NULL keeps the artifacts and the SQL agreeing), Parquet DOUBLE holds it natively (so NULL would
  *create* that disagreement and discard a value both consumers carry). **One combination is the
  exception and quoting made its worst case silent**: the rendering keys on the wire case while the
  writers key on the **declared** type, so a `numeric(p,s)` column arriving as `double_value` (the
  wire contract forbids it; nothing rejects it at ingest) is NULL in Parquet and `'NaN'` in SQL — as
  a *key* that was harmless only while the SQL was rejected, and `WHERE k = 'NaN'` now applies
  cleanly against a NULL baseline cell, matches nothing and diverges the mirror silently. So
  `hasUnrepresentableKey` becomes `unaddressableKeyReason` (the WARN names column and reason) and
  skips a non-finite `double_value` whose declared column materialises as a Parquet DECIMAL, asking
  `ParquetSchemaMapper.rendersAsParquetDecimal` — the field the writers build — rather than parsing
  the type name again (a bare `numeric` is Avro STRING and carries the token losslessly); an Avro-
  refused declaration answers *no* rather than throwing. **Round 4 widened the guard to
  `string_value`** — the identical silent divergence one wire case over, correcting the earlier claim
  that "a string is quoted anyway" settled it: quoting settles SQL *validity*, not which row the
  statement *addresses*. The counter stays one untagged series
  (`sql.generation.delta.records.skipped.unrepresentable_key`): the remedies differ and the WARN says
  which, but the alert is the same, and tagging an existing series breaks its dashboards. The **data**
  cells of that combination are newly silent and deliberately **not** guarded — that is coercion,
  #240's subject, which PR #232 reverted three times. The fix does not repair a file already written
  (`/sql-changes` returns stored objects): recovery is delete + generate or `reinit`. The DBF path's
  `formatValue` is guarded too through `ValueMapper.canonicalNonFinite` (widened, not copied — a
  second copy is exactly what #238 was), belt-and-braces: that branch is **unreachable** through its
  only production caller, since `DbfSqlGenerationStrategy` passes an empty `columnTypes` map — itself
  a finding (**#263**), which also makes the documented per-type handling dead. Mutation-proven (both
  branches removed → six methods red). No new metric, no REST, gRPC, proto, DTO, migration,
  configuration-key, S3-key or frontend change. See `docs/delta-client-v2-guide.md`,
  `docs/bitbi-integration.md`.
- poison-segment-backoff: One deterministically failing segment no longer stalls every other site's queue work (issue #243, after #212/#185). Both queues claim the **globally** oldest per-site head with `LIMIT 1` and the drain ended on the first `RuntimeException`, so a poison segment was offered first on every wake and nothing else in the fleet was produced; #212 made that stall permanent by holding pending segments back from retention. **V55** adds `plugin_sql_attempts`/`plugin_sql_retry_at` + `egress_attempts`/`egress_retry_at`: a failed attempt is recorded as a deferral (60 s doubling to 64x, the `delta.batch-parquet.retry-delay-seconds` shape) and the drain moves on. The backoff filters the **candidate row only** — the head-of-line `NOT EXISTS` is untouched, so the segment still blocks its own site (per-site seq order is a contract). **Poison-skip rejected with reasons**: a skip discards that batch's SQL/delta file permanently (nothing re-drives a stamped marker) while the causes are operator-repairable, i.e. exactly the silent loss #212 stopped — so attempts escalate reporting, and batch retention stays the one bounded ending. Loudness: `sql.generation.delta.segments.deferred` / **`delta.egress.errors`** (the `sql.generation.errors` twin the egress side lacked) per failure with a WARN; past `*-poison-after-attempts` (7 ≈ 1 h) an ERROR naming segment/site/seq/next retry and `delta.egress.segments.poisoned` / `sql.generation.delta.segments.poisoned` — a census, not a rate; all four registered at zero. `MemoryPressureAbortedException` (#181) is exempt and still ends the drain (systemic, self-repairing — #150/#162/#178). Deferral is a targeted UPDATE with the marker predicate (two replicas can attempt one segment since #164; also keeps clear of #245's whole-entity clobber), and a reinit clears the retry state with the marker. New keys `delta.egress.{retry-delay-seconds,poison-after-attempts}`, `plugin.sql-generation.delta-{retry-delay-seconds,poison-after-attempts}`, validated in their consuming beans (#185). Mutation-proven (rethrow instead of defer → four unit tests red). **Review round 1**: a drain stops after **one** deferral (continuing would walk the whole backlog during a systemic failure, drowning the poisoned signal and delaying recovery by the accumulated cooldowns); a failure during shutdown spends no attempt (#162); the #164 no-transaction guard is checked before the claim, so a wrapped drain reads as a wiring mistake rather than as poison data; a deferral matching no row is not reported; the ERROR names the threshold key; and the entity Javadoc's "keeps clear of #245" claim was corrected — the success path's whole-entity save still clobbers, now with four more columns, recorded as evidence on #245. No REST, gRPC, proto, DTO, S3-key or frontend change. See `docs/delta-client-v2-guide.md`, `docs/020-sql-generation-optimization.md`.
- adopt-path-side-effects: The loser of the SQL-generation unique claim stops reporting the winner's
  success as its own (issue #246, pre-existing, promoted out of the withdrawn findings inbox #242
  once #190/PR #236 made the race deterministic). Since #164 two workers can race
  `generateSqlForBatch` for one batch — `existsBySourceBatchId` runs before the S3 write and the
  INSERT after it, so both pass the guard and only `uk_sql_gen_source_batch` (V11) decides it.
  `persistOrAdoptExisting` already did the right durable thing (catch, delete **this** attempt's
  object, adopt the winner's row) but could not say which branch it returned through, so the loser
  ran the success path a second time: a second `SQL_GENERATION_COMPLETED` audit entry naming the key
  it had just deleted — the account sees two completed generations for one batch, one pointing at a
  dead object — and a second increment of `sql.generation.statements.*`, which describe the batch, so
  a raced batch was doubled on every dashboard. The generation was always the winner's; the lie was
  audit and metrics. **Make the branch visible rather than add an entry for it**: a private
  `ClaimOutcome(generation, adopted)`, and an adopted one returns early — no completion audit, no
  statement counters, one INFO. The ticket's alternative (an "adopted" audit entry) needs a new
  `PluginActionType`, i.e. stored data plus a `chk_plugin_audit_logs_action_type` migration, to say
  something the DoD asks to be silent about. The lost race is a counter instead, new
  **`sql.generation.claims.lost`**, registered at zero: *attempts that rendered and uploaded SQL for
  a batch another worker had already claimed* — wasted work, not a failure. **Review round 2
  corrected its operator guidance, the part worth keeping**: a steady rate is the ordinary state of a
  busy fleet (since #164 the claim's row lock is released by its own short transaction and
  `plugin_sql_at` is set only after the render, so overlapping drains pick the same global head
  deterministically and one always loses), so an alert on the first wording would have fired
  continuously against healthy operation — read it as a **waste rate** against completed generations;
  the durable claim that would stop the second render belongs to the queue, recorded on #243.
  `sql.generation.duration` and `sql.generation.semaphore.acquired` still move for both (both really
  rendered), and the loser's `SQL_GENERATION_STARTED` stands for the same reason — **its cost is
  documented rather than fixed**: this is now the only exit leaving a started entry with no terminal
  companion, so a raced batch reads as two "Generating SQL..." lines and one "SQL Generated", with
  `claims.lost` saying the unmatched line was a lost race rather than a crash; a terminal value of
  its own is a migration for a cosmetic asymmetry, filed as **#260**. Negative assertions are paired
  with a winner test that passes throughout (nothing else stops "never audited" being satisfied by a
  service that audits nothing) and both are mutation-proven. No migration (**V55 stays free**), no
  REST, gRPC, proto, DTO, configuration-key, cache, S3-key or frontend change; nothing renamed.
  See `docs/020-sql-generation-optimization.md`.
- absorbed-duplicates-marker: A ticket closed as absorbed carries the **`duplicate`** label (issue #230). Closing an issue folded into another auto-moves its card to `Done` via project 16's enabled `Item closed` workflow, so `Done` mixed a ticket closed by its own PR with one closed by somebody else's — eight such closes on 2026-08-19, four with the absorber still open, the sharpest #200 (`priority: high`, unfixed in production) in `Done` under a blocked #190, beside #143/#162/#204/#214 whose work really did land. A separate column was rejected because that same workflow overwrites it on every close (the workflows are readable via GraphQL, so this was checked rather than assumed) and because the columns are a state machine, not an outcome; "leave it in `Done`" is what the board already does. The label survives the auto-move, is one query (`gh issue list --state closed --label duplicate`), and was already triage's de-facto marker on #218/#229. **What it means was the real decision**: the durable fact *closed by another ticket's work* rather than "still unfixed" — the closer cannot know the second, and it decays (#200 labelled as unfixed on 19.08, #190 merged on 20.08) — so every absorbed close is labelled and a required closing comment names the absorber and its state. Backfilled as a **census, not #230's list**: review round 2 found six older folds unlabelled (#114→#112, #123/#124→#122, #160→#158, #163→#159, #176→#164) plus #241 under the still-open #239, so seventeen closes carry the label, three with the absorber open. Binds `/github-issue`, `/merge` and `/github-issue-runner`, and the label's description was rewritten to carry the decided meaning instead of GitHub's default. Review round 1 found the same misreading upstream: the follow-ups search rule told an agent a closed match is "almost always" stale, so a `duplicate`-closed ticket under an open absorber would have been read as shipped at the search step — that branch now names `duplicate` as the exception and the searches ask for labels; plus the absorbed close strips `status: *`, "do not move the card" became "do not move it elsewhere, but check it reached `Done`", and the dispatcher rule stopped covering closes that are not absorbed at all. Round 2 then found the invariant false (the census above), round 1's own label-aware search command a jq parse error, `/merge`'s trigger keyed on a literal (`folds #NNN`) that appears nowhere in this repository, the absorb-and-close step lacking any guard against closing a ticket somebody is currently working, and the dispatcher exclusion one case too wide. Docs and command files only.
- signed-nan-classification: `+NaN` and `-NaN` are classified `non_finite` and fold under the same
  identity as `NaN` (issue #238). Both places recognising PostgreSQL's non-finite spellings computed
  an **unsigned** form and then tested NaN against the **signed** one, so the two signed spellings
  fell through every non-finite branch. The cost was where that landed, not the fall-through:
  `BigDecimal` cannot parse the token, so `isMalformedDecimal` was true and the cell counted
  `delta.parquet.unrepresentable-decimals{reason=malformed}` — a series whose Javadoc defines it as
  "a client defect somebody has to fix", where the whole point of splitting it from `non_finite`
  (#215) was that the two want opposite responses from an operator; a signed NaN therefore paged
  someone to chase a bug that does not exist. Second consequence:
  `ChangelogFold.normalizeDecimal`, which canonicalises `nan`/`NaN` to `"NaN"` so one source row does
  not fold into two identities, returned the raw token for the signed spelling. PostgreSQL emits
  `NaN` unsigned, so a faithful round trip never reaches this — it is reachable because the token is
  whatever the client chose to send. One word in each of two places. **Deliberately not widened**:
  token classification only, not the destination-awareness #240 defers with three rounds of history
  as its warning. **Review corrected the justification, not the fix**: PostgreSQL rejects
  `'-NaN'::numeric`, so a signed NaN is evidence the *client* formats non-faithfully rather than a
  value the source held — still `non_finite`, since this pipeline cannot repair it. One pre-existing
  asymmetry it widens went to #240 rather than being fixed: `isNonFiniteToken` trims while
  `parseDecimal` is handed the raw token, so a padded *finite* token is written NULL and counted
  `malformed`. **Round 3 removed the duplication that was the defect** rather than its instance: the
  vocabulary lives once as package-private `ValueMapper.canonicalNonFinite`, with `ChangelogFold`
  calling it — two copies with nothing asserting they agree is how the identical slip came to exist
  in both — pinned by a test that also pins that the two infinities stay apart, which the shared
  canonicalisation must not collapse the way it deliberately collapses the NaN sign. Round 3 also
  caught this entry's own surfaces contradicting themselves: the guide called a signed NaN "a client
  to ask about" while **nothing** would tell an operator so (`malformed` no longer moves, `non_finite`
  is documented as nothing to repair, no log line prints the token) — the invisibility is deliberate
  and now stated in the guide and the counter's HELP. No REST, gRPC, proto, DTO, migration,
  configuration-key, metric-**name**, S3-key or frontend change. See
  `docs/delta-client-v2-guide.md` ("A value the column type cannot hold").
...
- sql-generation-config-fail-fast: An out-of-range value anywhere in the `plugin.sql-generation.*` block fails the application context at startup, and the dead async generator is gone (issue #185, folding **#210**, sequenced after #190). **Fail fast over a WARN is an owner decision recorded on the ticket** — argued once in `docs/020-sql-generation-optimization.md` ("One caveat on 'unbounded retry is safe'"): a GKE rolling update keeps old replicas serving while the rollout goes red, the WARN channel is proven unread (#174), and the silent failures are the expensive ones (`800` disables the heap guard; a negative value is an endless retry loop (#181) ending in silent data loss once retention passes the pending segments (#212); clamping ruled out by the ticket). **All five keys, two consuming constructors** (round 1 caught three-of-five with docs claiming the block): `SqlGenerationService` — `heap-threshold-percent` ∈ [1..100], `max-concurrent` >= 1, `semaphore-timeout-seconds` >= 1; `DeltaSqlSweepWorker` — `delta-max-concurrent` >= 1 (0 crash-looped through `ArrayBlockingQueue`'s message-less IAE) and `delta-sweep-ms` >= 1 (0 was accepted and busy-looped the sweep); shared package-private `PluginConfigValidation`, deliberately not shared wider. **Heap floor 1, not the 0 first shipped** (round 1): a live JVM's ceiling-rounded reading is never 0, so `> 0` refuses everything exactly like `-1` — and 0-as-off collides with the deployment's own "0 disables" convention; 100 stays #174's off-switch, pinned by `SqlGenerationStreamingTest`. Round 1 also killed the copied "0 permits deadlock the semaphore" text (bounded `tryAcquire`: the real signature is 120 s timeouts retried for ever) and made the tests pin "but was N" literally (a bare "0" was satisfied by static text); the named-key promise is scoped to well-formed integers — a malformed value dies earlier in `@Value` conversion naming the constructor parameter. Both boundaries of every range are `@ParameterizedTest`s in the two consumers' test classes. **Part 2**: `generateSqlForBatchAsync` deleted — one grep hit in `src/` (the declaration), Javadoc described the removed reinit flow (`PluginHistoryService`'s "SQL generation no longer triggered for reinit" comment stays as the record), and as a correctly-qualified `@Async` site the #195 guard kept it alive while #161-inventory readers counted it. Round 1 swept the stale prose the deletion exposed: `docs/reinit.md` (async regeneration documented as live, `sqlGenerationTriggered: true` example — it is always `false`), the controller's 202 comment, `BackgroundConnectionDemandTest`'s classification comment, `AsyncExecutorQualifierTest`'s 15-of-18 counts (13 of 15 now), `PluginHistoryServiceTest`'s T017 name, `PLUGIN_ID`'s wrong Javadoc. No migration (**V55 stays free**), no REST, gRPC, proto, DTO, metric, S3-key, configuration-key-**name** or frontend change. See `docs/020-sql-generation-optimization.md`.
...
- device-verify-expired-card: An expired browser verification reaches its dedicated **Code Expired** recovery card instead of being indistinguishable from an unknown code (issue #219). `DeviceAuthorizationService` already threw separate expired and not-found exceptions, but `DeviceAuthorizationController` converted both into 404, so the card the page already had was unreachable. The GET lookup keeps 404 for an absent authorization and answers **410 Gone** for expired; `DeviceVerifyPage` maps only 410 to `expired`, and every other status keeps the existing error card. The uncalled `denyAuthorizationDelete` export is deleted rather than leaving a second denial path beside the used POST action. **Review then found the card still unreachable by the likelier route**: the approve action kept mapping the same exception to 404, and the confirm card does not poll (`useVerifyInfo` has `retry: false`, `staleTime` 30 s, no `refetchInterval`), so a code valid on load and expired while the card sat open produced "Failed to authorize device. Please try again." — a retry that can never succeed. `POST /api/v1/device/verify` answers 410 too, and both action handlers route it to the same state through one `isExpiredResponse` predicate shared with the lookup. The `DELETE` denial is deliberately untouched: `deny` does not test expiry, so it cannot raise the condition, and its OpenAPI text stopped claiming otherwise. Two smaller ones: the 404 message dropped "or expired" — with expiry at 410 a 404 really is an unknown code, and since #211 that string is the *only* report — and the new `410` annotations no longer declare an `ErrorResponseDto` body the bodyless `.build()` responses never send. No migration, gRPC, proto, DTO, configuration-key, metric, S3-key, route-path or TanStack Query key change. See `docs/cr-device-verify-expired-card.md` and `docs/device-flow-client-guide.md`.
...
- prefix-walk-paged: The shared `ListObjectsV2` walk has a page-by-page form, and the orphan sweep uses it, so its heap is bounded by a page rather than by a site's history (issue #199, raised reviewing #198/#158). `S3PrefixLister.listAll` materialized a whole prefix before any caller could filter it, and `DeltaS3OrphanSweeper` then copied subsets of that into two more collections — one site's whole listing plus two derivations, on a scheduler thread, beside the checkpoint fold budget (#152/#178) and the Parquet scratch budget (#150), neither of which knows about it. The peak is one site's object count rather than the bucket's, which is why #158 shipped as it was; what makes it worth a ticket is that the sweep's **first deleting pass is by construction the largest listing this application ever takes** — its own premise is that superseded generations accumulated nightly for months with nothing reclaiming them. New `S3PrefixLister.forEachPage(client, bucket, prefix, Consumer<List<...>>)` returns `S3PrefixWalk(objectsRead, truncated)`; `listAll` is now that walk with a collecting consumer, so there is one implementation and the truncation contract of #122 is literally the same object in both. **The complete-listing callers keep their semantics unchanged**: the wipe compares every key against one instant and `requireCompleteKeys` (batch deletion, retention) needs the whole set or none, so both still call `listAll`. One behaviour the split *added*: the consumer runs **outside** the catch that classifies truncation, because this walk's consumer deletes objects and can therefore raise the very exception types the walk reads as "the listing stopped early" — a caller's failure reported as a short listing would be hidden behind a flag meaning the opposite. **What is bounded and what deliberately is not**: the listing is bounded by nothing, while the row set is bounded by what still exists, so the listing is consumed a page at a time and the row set stays **one read per site**, taken lazily on the first page that produces a candidate — no S3 call and no database call is added or removed, and this is a heap change and nothing else. The cost is stated rather than hidden: for a site whose prefix fits one page the ordering guard is unchanged, and beyond the first page it is weaker, with the **age window** carrying it; the checkpoint pointer read with those rows is older than the pages that follow, and an older pointer protects strictly more keys, so `couldStillBeAdopted` errs only towards keeping the object. A failed row read ends the **site**, every remaining page included, rather than the page that asked; the held-back and dry-run lines are accumulated and logged **once per site** with a ten-key sample, so an operator reads the same one line per prefix as before. `S3ChangelogSegmentStorage.walkPrefix` and `S3CheckpointStorage.walkPrefix` join their `listPrefix` twins — except on the segment side, where the materializing form had no caller left and is **removed** rather than kept as an invitation to restore the peak. **The delete is buffered rather than paged**, which review had to point out: a page and a `DeleteObjects` chunk are both 1000 keys, so deleting per page would have turned the sparse steady state into one round trip per page on a tick that walks every site prefix in the bucket — a site of two thousand pages holding four thousand thinly spread orphans would have taken up to two thousand round trips where the whole-listing version took four. Orphans are judged per page and queued, every full chunk goes out during the walk and the remainder in the `finally`, so the peak is under two chunks of keys and the bound this ticket is about is untouched. Pinned by tests on both sides: the walk hands pages over separately, keeps the pages already handed over on a mid-walk failure, and does **not** swallow a consumer's `S3Exception`; the sweep sends orphans thinly spread over two pages in **one** round trip, flushes a full chunk mid-walk, reads the row set once for three pages, and records "rows" *before* "walk-finished", which a materializing implementation cannot do. No REST, gRPC, proto, DTO, migration, configuration-key, metric-name, S3-key or frontend change. See `docs/delta-client-v2-guide.md` ("Objects no row references are reclaimed").
