...
## Recent Changes
- sql-generation-test-isolation: The two plugin SQL-generation integration classes assert on the generation they produced instead of sampling every generation a site has (issue #159, folding #163). `plugin_sql_generations` is never named by `test-data.sql` — it is emptied only as a side effect of three `ON DELETE CASCADE`s, for the accounts and sites its `LIKE` filters match, and only at the instant the script runs — so `findBySiteId(SITE)` + `hasSize(n)` + `get(0)` counted rows no test method owns. The same line was seen failing **both ways** ("was 2" when something else wrote for the site during the method, "was 0" when the test's own row was not visible yet), so narrowing the query alone is not the fix: assertions now name the source batch (`findBySourceBatchId`, unique) **and** wait for it with awaitility; where `hasSize(1)` stood in for "the baseline batch produced none" that is said directly and scoped, and negative assertions hold for half a second instead of sampling once. `BitBiDeltaSqlIntegrationTest` keeps the day-wide `findBySiteIdAndCreatedAtAfter` call its `/sql-changes` ordering assertions are about and filters it to the batches the method seeded; `SqlGenerationIntegrationTest`'s `findAll()).isEmpty()` is scoped to its own batch. **The load-bearing change is one line of `application-test.yml`**: `plugin.sql-generation.delta-sweep-ms: 3600000`, the slow-sweep treatment `delta.egress.sweep-ms` and `delta.batch-parquet.sweep-ms` already had and 026 never gave this queue — at 60s the tick fired in every cached Spring context and `DeltaSqlQueueService.processNextPending()` renders inside its transaction (S3 included), so a finished class's context could hold row locks on `changelog_segments` while the next class's `@Sql("/test-data.sql")` deleted those rows, which is the `ScriptStatementFailedException` that took two follow-on methods down with the first. The explicit `DeltaSqlSweepWorker.wake()` (BATCH_COMPLETED, reinit) is untouched. `clearAppSettings()` moves to `AbstractIntegrationTest` and is joined by `clearPluginSqlGenerations()`, called in both `@BeforeEach`es and pinned in the #119 leftover-then-clear shape. The eight same-shaped assertions in `PluginHistoryIntegrationTest` sit inside `@Disabled` nested classes and were left alone. Test-only — no production code, REST, gRPC, DTO, migration, production configuration-key, metric, S3-key or frontend change.
