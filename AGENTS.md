...
## Recent Changes
- s3-presence-tristate: A read denial and an absent object are two different answers again, at the call site that acts on them (issue #157, filed reviewing #154, made durable by #149). `S3CheckpointStorage.exists` returned `false` for a genuine 404 and for a 403 alike. The 404-as-403 half is correct and stays — least-privilege IAM (`GetObject`/`PutObject`, no `s3:ListBucket`) answers **403** for a key that does not exist — but a blanket read denial on keys that *do* exist answers 403 too, so one IAM or bucket-policy change made the frame read as absent for every site in the same tick and tripped `delta.checkpoint.builds.aborted`, whose contract is "aborts that do not repair themselves". **#149 raised that from noisy to irreversible**: it decides `history_gone` by the same call, spends one `checkpoints.materialize_attempts` per tick, and after `DELTA_CHECKPOINT_MAX_MATERIALIZE_ATTEMPTS` (5) nights a segment-less site is named by neither work-list query and does not recover when the permission does. New `presence(String)` → `PRESENT|ABSENT|UNKNOWN` combines the ticket's first two options because neither alone works: the tri-state without the probe would answer `UNKNOWN` for every missing key on a least-privilege deployment, the probe without the tri-state would still collapse a real outage into a lie. A denied HEAD is resolved by a ranged `GetObject` of the first byte (`NoSuchKey` = gone, success = denied-but-present); only a probe denied too yields `UNKNOWN`, and `416` reads as present (a zero-length object has no first byte). `CheckpointService` **skips** a site whose frame presence is unknown — nothing folded, uploaded, saved or counted, no attempt spent, retried next tick — deliberately not on `builds.aborted`, since this repairs itself. New counter **`delta.s3.read-denied`** (registered in `S3CheckpointStorage` over an injected `MeterRegistry`, the `CheckpointGivenUpMetrics` shape, so infrastructure does not depend on `delta.application`) counts the **unresolved** denial only — every HEAD 403 would be one per missing key and pure noise, which is why the ticket's `delta.s3.head-denied` name was not used. `deltaExists`/`frameExists` → `deltaPresence`/`framePresence`; `exists` stays as `presence == PRESENT` for the integration suite. Parquet Export drops a delta row only on a **known** absence. The `lossy_refold` caveat is removed rather than reworded in the guide and in `DeltaMetrics.checkpointBuildAborted`. Proven against LocalStack for the half a mock cannot (ranged probe reads a real object through a denied HEAD; a missing key really answers `NoSuchKey`), with only the denial injected via `DelegatingS3Client`, since LocalStack community enforces no IAM. No REST, gRPC, proto, DTO, migration, configuration-key, S3-key or frontend change. See `docs/delta-client-v2-guide.md` ("S3 will not say", Metrics), `docs/parquet-export-plugin-guide.md`.
