/**
 * Monitoring design tokens — single shared source (024, T001).
 *
 * Promoted verbatim from features/delta-sync/model/tokens.ts (023, F4); values
 * originate from design_handoff_delta_sync/README.md "Design Tokens".
 *
...
/**
 * Severity keys of the monitoring status palette. `features/delta-sync` maps its
 * `SyncSeverity` onto these; other surfaces generalize them as
 * success / warning / error / stalled.
 */
export type MonitoringSeverity = 'healthy' | 'elevated' | 'critical' | 'stalled';
...
export const severityTokens: Record<MonitoringSeverity, SeverityToken> = {
