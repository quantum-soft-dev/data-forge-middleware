/**
 * SqlContentViewerDialog Component
 *
 * Modal dialog for viewing paginated SQL content for a generation.
 * User-facing version that uses JWT authentication.
 */
...
import { useSqlContentQuery, useDownloadSqlFileMutation } from '../api/batchSqlQueries'
import type { BatchSqlStatus } from '../model/types'

interface SqlContentViewerDialogProps {
  pluginId: string
...
export function SqlContentViewerDialog({
  pluginId,
  batch,
  isOpen,
  onClose,
}: SqlContentViewerDialogProps) {
  ...
  const downloadMutation = useDownloadSqlFileMutation()
  ...
  const handleDownload = () => {
    if (generationId) {
      downloadMutation.mutate({ pluginId, generationId })
    }
  }

  // Reset page when dialog opens with new batch
  ...
  if (!batch || !batch.generationId) return null
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            {batch.siteDomain} - Batch {batch.batchId.slice(0, 8)}...
          </DialogDescription>
        </DialogHeader>
