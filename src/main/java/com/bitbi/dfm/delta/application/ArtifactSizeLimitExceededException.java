 ...
 *
 * <p>Shared by the completed-batch writer, the checkpoint snapshot writer (issue #112) and the
 * checkpoint-frame codec (issue #126): each streams to local disk, so each needs the same guard
 * against a single artifact filling the node.</p>
 *
